  ...
  function generaMatrizInputs(){
    ...
    for(i=1;i<=filas;i++){
      ...
      matriz+=aux+' ≤  <input type="text" value="0" name="'+'R'+i+'" id="'+'R'+i+'" onClick="this.select();" required="required"  /><br /><br />';aux='';
    
    }
    
    document.getElementById('matriz').innerHTML='<p>Restricciones:</p>'+matriz;ceros+=' ≥ 0';
  ...
  function encuentraPivoteJ(){
    ...
    var variables=parseInt(document.getElementById('nd').value);
    var restricciones=parseInt(document.getElementById('nr').value);
    var itemFila=matriz[restricciones+1][1];pivoteJ=1;
    ...
    if(objetivo=='max'){
      ...
      for(j=1;j<=variables;j++){
        ...
        if(matriz[restricciones+1][j]<itemFila&&matriz[restricciones+1][j]!=0){
          ...
          itemFila=matriz[restricciones+1][j];pivoteJ=j;
